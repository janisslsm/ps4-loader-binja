...
def load_nids(location, nids={}):
    try:
        ...
        nidsDialog = OpenFileNameField(
            "Select aerolib.csv", "aerolib.csv|*.csv|All files (*.*)|*.*"
        )
...
class PS4ELF(BinaryView):
    ...
    def init(self):
        self.nids = load_nids(
            os.path.dirname(os.path.abspath(__file__)) + "/aerolib.csv"
        )
    ...
    def parse_sce(self):
            ...
            if segm.name() in [
                ...
                if segm.name() not in ["DYNAMIC", "GNU_EH_FRAME"]:
                    self.add_auto_segment(
                        address, size, segm.OFFSET, segm.FILE_SIZE, segm.type()
                    )
                    self.add_auto_section(
                        segm.name(), address, size, segm.section_type()
                    )
                    ...
                    for entry in range(int(dynamicsize / 0x10)):
                        self.define_data_var(
                            address + (entry * 0x10), self.parse_type_string("int")[0]
                        )
                        self.set_comment_at(
                            address + (entry * 0x10),
                            Dynamic(self).process(stubs, modules, libraries),
                        )
            ...
            if segm.name() == "SCE_DYNLIBDATA":
                ...
                try:
                    ...
                    for entry in range(int(Dynamic.SYMTABSZ / 0x18)):
                        ...
                        self.set_comment_at(
                            location + (entry * 0x18), SCESymbol(self).process(symbols)
                        )
                ...
                try:
                    ...
                    for entry in range(int((Dynamic.SYMTABSZ - 0x30) / 0x18)):
                        SCESymbol(self).resolve(
                            location + (entry * 0x18), self.nids, symbols[entry][1]
                        )
                ...
                try:
                    ...
                    members = [
                        ("offset", "Offset (String Index)", 0x8),
                    ...
                    alphabet = {
                        character: index for index, character in enumerate(base64)
                    }
                    ...
                    for entry in range(int(Dynamic.JMPTABSZ / 0x18)):
                        ...
                        self.set_comment_at(
                            location + (entry * 0x18),
                            Relocation(self).resolve(
                                alphabet, self.nids, symbols, lids
                            ),
                        )
                ...
                try:
                    ...
                    for entry in range(int(Dynamic.RELATABSZ / 0x18)):
                        ...
                        self.set_comment_at(
                            location + (entry * 0x18),
                            Relocation(self).process(self.nids, symbols),
                        )
                ...
                try:
                    ...
                    for entry in range(int(dynamicsize / 0x10)):
                        ...
                        self.set_comment_at(
                            dynamic + (entry * 0x10),
                            Dynamic(self).comment(address, stubs, modules, libraries),
                        )
        ...
        if self.E_START_ADDR != 0:
            self.define_auto_symbol(
                Symbol(SymbolType.FunctionSymbol, self.E_START_ADDR, "_start")
            )
...
class Relocation:
    ...
    def process(self, nids, symbols):
        ...
        if self.INFO > Relocation.R_X86_64_ORBIS_GOTPCREL_LOAD:
            ...
            if self.type() != "R_X86_64_DTPMOD64":
                symbol = next(
                    value for key, value in enumerate(symbols) if key + 2 == self.INDEX
                )[1]
    ...
    def resolve(self, alphabet, nids, symbols, libraries):
        ...
        try:
            ...
            func_type = Type.function(Type.int(8, False), [], variable_arguments=True)
            func.type = func_type
...
class Dynamic:
    ...
    def comment(self, address, stubs, modules, libraries):
        ...
        if self.TAG in [Dynamic.DT_NEEDED, Dynamic.DT_SONAME]:
            ...
            self.bv.define_auto_symbol(
                Symbol(SymbolType.DataSymbol, address, ".dynstr")
            )
            ...
            self.bv.define_auto_symbol(
                Symbol(SymbolType.DataSymbol, address, ".dynsym")
            )
            ...
            Dynamic.DT_SCE_NEEDED_MODULE,
            Dynamic.DT_SCE_IMPORT_LIB,
            Dynamic.DT_SCE_IMPORT_LIB_ATTR,
            Dynamic.DT_SCE_EXPORT_LIB,
            Dynamic.DT_SCE_EXPORT_LIB_ATTR,
            Dynamic.DT_SCE_MODULE_INFO,
            Dynamic.DT_SCE_MODULE_ATTR,
            Dynamic.DT_SCE_FINGERPRINT,
    ...
    def process(self, stubs, modules, libraries):
        ...
        if self.TAG == Dynamic.DT_INIT:
            ...
            if Dynamic.INIT > 0:
                self.bv.define_auto_symbol(
                    Symbol(SymbolType.FunctionSymbol, Dynamic.INIT, ".init")
                )
            ...
            if Dynamic.FINI > 0:
                self.bv.define_auto_symbol(
                    Symbol(SymbolType.FunctionSymbol, Dynamic.FINI, ".fini")
                )
            ...
            self.bv.define_auto_symbol(
                Symbol(SymbolType.DataSymbol, Dynamic.GOT, ".got.plt")
            )
...
class Segment:
    ...
    def name(self):
        ...
        return {
            ...
            if self.flags() == (Segment.SEGPERM_EXEC | Segment.SEGPERM_READ)
            else "DATA",
    ...
    def type(self):
        return {
            Segment.PT_LOAD: SegmentFlag.SegmentContainsCode
            | SegmentFlag.SegmentReadable
            | SegmentFlag.SegmentWritable
            | SegmentFlag.SegmentExecutable
            if self.flags() == (Segment.SEGPERM_EXEC | Segment.SEGPERM_READ)
            else SegmentFlag.SegmentContainsData | SegmentFlag.SegmentReadable,
            Segment.PT_DYNAMIC: SegmentFlag.SegmentContainsData
            | SegmentFlag.SegmentReadable,
            Segment.PT_INTERP: SegmentFlag.SegmentContainsData
            | SegmentFlag.SegmentReadable,
            Segment.PT_NOTE: SegmentFlag.SegmentContainsData
            | SegmentFlag.SegmentReadable,
            Segment.PT_PHDR: SegmentFlag.SegmentContainsCode
            | SegmentFlag.SegmentReadable
            | SegmentFlag.SegmentWritable
            | SegmentFlag.SegmentExecutable,
            Segment.PT_TLS: SegmentFlag.SegmentContainsData
            | SegmentFlag.SegmentReadable,
            Segment.PT_SCE_DYNLIBDATA: SegmentFlag.SegmentContainsData
            | SegmentFlag.SegmentReadable,
            Segment.PT_SCE_PROCPARAM: SegmentFlag.SegmentContainsData
            | SegmentFlag.SegmentReadable,
            Segment.PT_SCE_MODULEPARAM: SegmentFlag.SegmentContainsData
            | SegmentFlag.SegmentReadable,
            Segment.PT_SCE_RELRO: SegmentFlag.SegmentContainsData
            | SegmentFlag.SegmentReadable,
            Segment.PT_GNU_EH_FRAME: SegmentFlag.SegmentContainsData
            | SegmentFlag.SegmentReadable,
            Segment.PT_GNU_STACK: SegmentFlag.SegmentContainsData
            | SegmentFlag.SegmentReadable,
        }.get(self.TYPE, SegmentFlag.SegmentReadable | SegmentFlag.SegmentContainsData)

    def section_type(self):
        return {
            Segment.PT_LOAD: SectionSemantics.ReadOnlyCodeSectionSemantics
            if self.flags() == (Segment.SEGPERM_EXEC | Segment.SEGPERM_READ)
